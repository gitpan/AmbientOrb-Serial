# This Makefile is for the AmbientOrb::Serial extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/AmbientOrb/Serial.pm]
#     AUTHOR => q[Lyle Hayhurst <sozin@cpan.org>]
#     NAME => q[AmbientOrb::Serial]
#     PL_FILES => {  }
#     PREREQ_PM => { Test::More=>q[0], Class::Accessor=>q[0], Win32::SerialPort=>q[0.19], Carp=>q[0] }
#     VERSION_FROM => q[lib/AmbientOrb/Serial.pm]
#     clean => { FILES=>q[AmbientOrb-Serial-*] }
...
FULLEXT = AmbientOrb\Serial
BASEEXT = Serial
PARENT_NAME = AmbientOrb
DLBASE = $(BASEEXT)
...
INST_LIBDIR      = $(INST_LIB)\AmbientOrb
INST_ARCHLIBDIR  = $(INST_ARCHLIB)\AmbientOrb
...
TO_INST_PM = lib/AmbientOrb/Serial.pm

PM_TO_BLIB = lib/AmbientOrb/Serial.pm \
...
DISTNAME = AmbientOrb-Serial
DISTVNAME = AmbientOrb-Serial-0.01
...
Serial.def: Makefile.PL
	$(PERLRUN) -MExtUtils::Mksymlists \
     -e "Mksymlists('NAME'=>\"AmbientOrb::Serial\", 'DLBASE' => '$(BASEEXT)', 'DL_FUNCS' => {  }, 'FUNCLIST' => [], 'IMPORTS' => {  }, 'DL_VARS' => []);"
...
clean :: clean_subdirs
	-$(RM_RF) AmbientOrb-Serial-* ./blib $(MAKE_APERL_FILE) $(INST_ARCHAUTODIR)/extralibs.all $(INST_ARCHAUTODIR)/extralibs.ld perlmain.c tmon.out mon.out so_locations pm_to_blib *$(OBJ_EXT) *$(LIB_EXT) perl.exe perl perl$(EXE_EXT) $(BOOTSTRAP) $(BASEEXT).bso $(BASEEXT).def lib$(BASEEXT).def $(BASEEXT).exp $(BASEEXT).x core core.*perl.*.? *perl.core core.[0-9] core.[0-9][0-9] core.[0-9][0-9][0-9] core.[0-9][0-9][0-9][0-9] core.[0-9][0-9][0-9][0-9][0-9]
...
realclean purge ::  clean realclean_subdirs
	...
	$(RM_RF) $(DISTVNAME)
	$(RM_F)  $(MAKEFILE_OLD) $(FIRST_MAKEFILE) blib\lib\AmbientOrb\Serial.pm
...
metafile :
	...
	$(NOECHO) $(ECHO) "name:         AmbientOrb-Serial" >> META.yml
	$(NOECHO) $(ECHO) "version:      0.01" >> META.yml
	$(NOECHO) $(ECHO) "version_from: lib/AmbientOrb/Serial.pm" >> META.yml
	$(NOECHO) $(ECHO) "installdirs:  site" >> META.yml
	$(NOECHO) $(ECHO) "requires:" >> META.yml
	$(NOECHO) $(ECHO) "    Carp:                          0" >> META.yml
	$(NOECHO) $(ECHO) "    Class::Accessor:               0" >> META.yml
	$(NOECHO) $(ECHO) "    Test::More:                    0" >> META.yml
	$(NOECHO) $(ECHO) "    Win32::SerialPort:             0.19" >> META.yml
...
TEST_FILE = test.pl
TEST_FILES = t\00-load.t t\01-color.t t\01-connect.t t\02-color2ascii.t t\boilerplate.t t\pod-coverage.t t\pod.t
TESTDB_SW = -d
...
ppd:
	...
	$(NOECHO) $(ECHO) "    <ABSTRACT>The great new AmbientOrb::Serial!</ABSTRACT>" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "    <AUTHOR>Lyle Hayhurst &lt;sozin@cpan.org&gt;</AUTHOR>" >> $(DISTNAME).ppd
	...
	$(NOECHO) $(ECHO) "        <DEPENDENCY NAME=\"Win32-SerialPort\" VERSION=\"0,19,0,0\" />" >> $(DISTNAME).ppd
...
pm_to_blib: $(TO_INST_PM)
	$(NOECHO) $(PERLRUN) -MExtUtils::Install -e "pm_to_blib({@ARGV}, '$(INST_LIB)\auto', '$(PM_FILTER)')"\
	  lib/AmbientOrb/Serial.pm blib\lib\AmbientOrb\Serial.pm 
	$(NOECHO) $(TOUCH) $@
